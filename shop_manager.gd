...
func _find_card_slot_in_shop(card_id: String) -> int:
    ...
    for i in range(shop_area.get_child_count()):
        var child = shop_area.get_child(i)
        if child.name != "ShopAreaLabel" and child.has_meta("card_id"):
            if child.get_meta("card_id") == card_id:
                return i - 1  # Subtract 1 for label
    return -1
